...
type Payment struct {
	...
	PaymentAt    time.Time `boil:"payment_at" json:"payment_at" toml:"payment_at" yaml:"payment_at"`
...
var PaymentTableColumns = struct {
	...
	CategoryName: "payments.category_name",
	PaymentType:  "payments.payment_type",
	PaymentAt:    "payments.payment_at",
...
var PaymentWhere = struct {
	...
	PaymentAt    whereHelpertime_Time
	...
	Label:        whereHelperstring{field: "\"payments\".\"label\""},
	CategoryName: whereHelperstring{field: "\"payments\".\"category_name\""},
	PaymentType:  whereHelperstring{field: "\"payments\".\"payment_type\""},
	PaymentAt:    whereHelpertime_Time{field: "\"payments\".\"payment_at\""},
	...
	Cost:         whereHelperint{field: "\"payments\".\"cost\""},
}
...
var PaymentRels = struct {
	...
	PaymentPaymentType   string
}{
	User:                 "User",
	CategoryNameCategory: "CategoryNameCategory",
	PaymentPaymentType:   "PaymentPaymentType",
}
...
type paymentR struct {
	...
	PaymentPaymentType   *PaymentType `boil:"PaymentPaymentType" json:"PaymentPaymentType" toml:"PaymentPaymentType" yaml:"PaymentPaymentType"`
}
...
func (r *paymentR) GetCategoryNameCategory() *Category {
	if r == nil {
		return nil
	}
...
func (r *paymentR) GetPaymentPaymentType() *PaymentType {
	if r == nil {
		return nil
	}
	return r.PaymentPaymentType
}
...
var (
	paymentAllColumns            = []string{"id", "label", "category_name", "payment_type", "payment_at", "created_at", "updated_at", "user_id", "cost"}
	paymentColumnsWithoutDefault = []string{"label", "category_name", "payment_type", "payment_at", "created_at", "updated_at", "cost"}
	paymentColumnsWithDefault    = []string{"id", "user_id"}
...
func (o *Payment) CategoryNameCategory(mods ...qm.QueryMod) categoryQuery {
	queryMods := []qm.QueryMod{
...
// PaymentPaymentType pointed to by the foreign key.
func (o *Payment) PaymentPaymentType(mods ...qm.QueryMod) paymentTypeQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"label\" = ?", o.PaymentType),
	}

	queryMods = append(queryMods, mods...)

	return PaymentTypes(queryMods...)
}
...
// loaded structs of the objects. This is for an N-1 relationship.
func (paymentL) LoadCategoryNameCategory(ctx context.Context, e boil.ContextExecutor, singular bool, maybePayment interface{}, mods queries.Applicator) error {
	var slice []*Payment
	var object *Payment
	...
	if singular {
		...
		foreign.R.CategoryNamePayments = append(foreign.R.CategoryNamePayments, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.CategoryName == foreign.Classification {
				...
				foreign.R.CategoryNamePayments = append(foreign.R.CategoryNamePayments, local)
				break
			}
		}
	}
...
// LoadPaymentPaymentType allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (paymentL) LoadPaymentPaymentType(ctx context.Context, e boil.ContextExecutor, singular bool, maybePayment interface{}, mods queries.Applicator) error {
	var slice []*Payment
	var object *Payment
	...
	if singular {
		...
		args = append(args, object.PaymentType)
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if a == obj.PaymentType {
					continue Outer
				}
			}

			args = append(args, obj.PaymentType)
	...
	query := NewQuery(
		qm.From(`payment_types`),
		qm.WhereIn(`payment_types.label in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load PaymentType")
	}

	var resultSlice []*PaymentType
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice PaymentType")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for payment_types")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for payment_types")
	}
	...
	if singular {
		foreign := resultSlice[0]
		object.R.PaymentPaymentType = foreign
		if foreign.R == nil {
			foreign.R = &paymentTypeR{}
		}
		foreign.R.Payments = append(foreign.R.Payments, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.PaymentType == foreign.Label {
				local.R.PaymentPaymentType = foreign
				if foreign.R == nil {
					foreign.R = &paymentTypeR{}
				}
				foreign.R.Payments = append(foreign.R.Payments, local)
...
// Sets o.R.CategoryNameCategory to related.
// Adds o to related.R.CategoryNamePayments.
func (o *Payment) SetCategoryNameCategory(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Category) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	if related.R == nil {
		related.R = &categoryR{
			CategoryNamePayments: PaymentSlice{o},
		}
	} else {
		related.R.CategoryNamePayments = append(related.R.CategoryNamePayments, o)
	}
...
// Sets o.R.PaymentPaymentType to related.
// Adds o to related.R.Payments.
func (o *Payment) SetPaymentPaymentType(ctx context.Context, exec boil.ContextExecutor, insert bool, related *PaymentType) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	o.PaymentType = related.Label
	if o.R == nil {
		o.R = &paymentR{
			PaymentPaymentType: related,
		}
	} else {
		o.R.PaymentPaymentType = related
	}

	if related.R == nil {
		related.R = &paymentTypeR{
			Payments: PaymentSlice{o},
		}
	} else {
		related.R.Payments = append(related.R.Payments, o)
