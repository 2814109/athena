...
type accountR struct {
	AccountAccountType *AccountType     `boil:"AccountAccountType" json:"AccountAccountType" toml:"AccountAccountType" yaml:"AccountAccountType"`
...
func (r *accountR) GetAccountAccountType() *AccountType {
	if r == nil {
		return nil
	}
	return r.AccountAccountType
}

func (r *accountR) GetTransactions() TransactionSlice {
	if r == nil {
		return nil
	}
...
// AccountAccountType pointed to by the foreign key.
func (o *Account) AccountAccountType(mods ...qm.QueryMod) accountTypeQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"label\" = ?", o.AccountType),
	}
...
// LoadAccountAccountType allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (accountL) LoadAccountAccountType(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAccount interface{}, mods queries.Applicator) error {
	var slice []*Account
	var object *Account
	...
	if singular {
		...
		args = append(args, object.AccountType)
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if a == obj.AccountType {
					continue Outer
				}
			}

			args = append(args, obj.AccountType)
	...
	query := NewQuery(
		qm.From(`account_types`),
		qm.WhereIn(`account_types.label in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for account_types")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for account_types")
	}
	...
	if singular {
		...
		object.R.AccountAccountType = foreign
		if foreign.R == nil {
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.AccountType == foreign.Label {
				local.R.AccountAccountType = foreign
				if foreign.R == nil {
					foreign.R = &accountTypeR{}
				}
...
func (accountL) LoadTransactions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAccount interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`transactions.account_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets o.R.AccountAccountType to related.
// Adds o to related.R.Accounts.
func (o *Account) SetAccountAccountType(ctx context.Context, exec boil.ContextExecutor, insert bool, related *AccountType) error {
	var err error
	if insert {
	...
	o.AccountType = related.Label
	if o.R == nil {
		o.R = &accountR{
			AccountAccountType: related,
		}
	} else {
		o.R.AccountAccountType = related
	}
...
// Removes related items from R.Transactions (uses pointer comparison, removal does not keep order)
// Sets related.R.Account.
func (o *Account) RemoveTransactions(ctx context.Context, exec boil.ContextExecutor, related ...*Transaction) error {
	if len(related) == 0 {
		return nil
	}
